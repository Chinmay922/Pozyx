...
String serialPort = "/dev/tty.usbmodem1431";      // change this to your COM port 
...
int offset_x = 30; //changes blue border
int offset_y = 30; //changes blue border
float pixel_per_mm = 0.5; //changes how many grids fit in screen ?
int border = 500; //changed from 500 to adjust origin/layout of graph on screen
int thick_mark = 500; //changes width of grid boxes, changed from 500
int device_size = 5;
...
void setup(){
  size(1000, 700, P3D); //changes how much window fills up computer screen, width, height
...
void draw(){
  background(126,161,172); //border color, blue
  fill(0,0,0);
  text("(c) Pozyx Labs", width-100, 20); //changes text in top right corner  
  drawMap();
}
  
  
//**** begin function drawMap ***//
  
void drawMap(){
  
  int plane_width =  width - 2 * offset_x; //width of white space?
  int plane_height = height - 2 * offset_y; //height of white space?
  // draw the plane
  stroke(0);
  fill(255);
  ...
  // draw the grid
  strokeWeight(1); //in pixels, where 1 is relatively thin
  //draws horz lines
  stroke(200);
  ...
  stroke(0);
  fill(0);
  drawArrow(0, 0, 50, 0.); //center x, center y, length, angle
  drawArrow(0, 0, 50, 90.);
  pushMatrix();
  rotateX(radians(180));
  text("X", 55, 5);  //string to print, x coord, y coord
  text("Y", -3, -55);
  popMatrix();  
